...
if [[ ! -d "$FILTERED_DIR" ]]; then
  echo "[filtered-video] Filtered snapshot directory not found: $FILTERED_DIR" >&2
  echo "[filtered-video] Run linear stability first:" >&2
  echo "[filtered-video]   bash $ROOT_DIR/pipeline/05_linear_stability.sh" >&2
  exit 1
fi

echo "[filtered-video] Rendering filtered frames with the common plot script"
bash "$ROOT_DIR/pipeline/01_frames.sh" "$FILTERED_DIR" "$PLOT_ROOT" "$SNAPSHOT_INDEX"
