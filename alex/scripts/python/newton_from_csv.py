...
def forcing_library_path() -> Path:
    ...
    candidates = [
        root_dir / "cpu" / "build" / "libomega_forcing.dylib",
        root_dir / "cpu" / "build" / "libomega_forcing.so",
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("alex/cpu/configs/pulsation.cfg"), help="Alex config for Re and wall velocities")
