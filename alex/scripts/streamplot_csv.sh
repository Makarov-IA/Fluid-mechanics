#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PY_SCRIPT="$ROOT_DIR/scripts/streamplot_csv.py"

# =========================
# USER SETTINGS
# =========================
# Edit this block and then run:
#   bash alex/scripts/streamplot_csv.sh

INPUT_CSV="$ROOT_DIR/stationary_snapshot/newton_equilibrium_from_stationary.csv"
OUTPUT_PNG="$ROOT_DIR/stationary_snapshot/newton_equilibrium_from_stationary.png"

# INPUT_CSV="$ROOT_DIR/stationary_snapshot/before_oscillation_t2_32001_step232000.csv"
# OUTPUT_PNG="$ROOT_DIR/stationary_snapshot/before_oscillation_streamplot.png"

DENSITY="1.6"
DPI=180
TITLE="Before oscillation, t=2.32001"

# Set DRAW_CONTOURS=false to hide psi contour lines.
DRAW_CONTOURS=true

# Add raw extra arguments here if needed.
EXTRA_ARGS=(
)
# =========================

if [[ ! -f "$INPUT_CSV" ]]; then
  echo "[streamplot] CSV not found: $INPUT_CSV" >&2
  exit 1
fi

CMD=(
  python3 "$PY_SCRIPT"
  "$INPUT_CSV"
  -o "$OUTPUT_PNG"
  --density "$DENSITY"
  --dpi "$DPI"
  --title "$TITLE"
)

if [[ "$DRAW_CONTOURS" != "true" ]]; then
  CMD+=(--no-contours)
fi

if [[ ${#EXTRA_ARGS[@]} -gt 0 ]]; then
  CMD+=("${EXTRA_ARGS[@]}")
fi

echo "[streamplot] input : $INPUT_CSV"
echo "[streamplot] output: $OUTPUT_PNG"
"${CMD[@]}"
